function [] = rffexhibits_function_oracle(gamma, trnwin, stdize, demean, RFF_stdize, CVType)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% Local path
thisRFF_datadir = strcat('./individual-files_stdize_','oracle', '_RFFs_', num2str(RFF_stdize),'/');
...
combined_data_save_path = ['./combined_data_RFF_',num2str(RFF_stdize),'_','oracle','/'];
...
dfOpt_collect = Bnrm_collect; % corresponding df
...
if isfile(filename)
    ...
    for s = 1:nSim
        ...
        load(filename, 'Yprd', 'Bnrm', 'DfOpt');

        Yprd_collect(:,:,:,s)   = Yprd;
        Bnrm_collect(:,:,:,s)   = Bnrm;
        dfOpt_collect(:,:,:,s)   = DfOpt; % T x P x NL
    end

    clearvars Yprd Bnrm DfOpt
    Yprd = Yprd_collect;
    Bnrm = Bnrm_collect;
    ...
    dfOpt_summary = nanmean(squeeze(dfOpt_collect(:,:,:,:)),4); % mean along 4th dimension?
end
...
for ss=1:nSub
    ...
    if isfile(performance_filename)
        ...
        save(performance_filename,'locev','ER','SR', 'vol','IR','IRt','alpha','R2',...
            ...
            'Bnrmbar','timing', 'pihat', 'Yprd','dfOpt_summary', '-v7.3')
    end
